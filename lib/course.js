 ...
 */
  parse(text) {
    ...
    if (parsed.sections) {
      Object.keys(parsed.sections).forEach((s) => {this.sections[s] = []});
      if (typeof parsed.sections[0] === Object) parsed.sections.forEach((s, ind) => {s[ind].forEach((w) => {this.sections[ind].push(w)})})
    }
 ...
 */
  addWorkout(workout) {
    ...
    if (typeof workout.section !== undefined) this.sections[workout.section].push(workout);
  }
