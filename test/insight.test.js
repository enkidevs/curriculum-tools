...
describe('Insight', () => {
  ...
  test('parses the content section from the raw insight text', () => {
    ...
    contentArr.forEach(content => {
      expect(possibleContents).toContain(content)
    })
  })
  ...
  test('parses the Revision Question answers', () => {
    const revisionQuestionAnswersArr = testInsights
      .map(insight => insight.getRevisionQuestion())
      .filter(Boolean)
      .map(revisionQuestion => revisionQuestion.answers)

    revisionQuestionAnswersArr.forEach(insight => {
      expect(insight.getRevisionQuestion()).toBeDefined()
    })

    expect(revisionQuestionAnswersArr).toEqual(expect.arrayContaining([
      [ '`undefined`',
        '`5`',
        '`false`',
        '`error`' ],
