...
module.exports = class Course extends Content {
  ...
  parse(text) {
    if (text.length === 0) {
      throw new Error('Passed in empty readme text');
    }
    let parsed = yaml.safeLoad(text);

    if (parsed.sections) {
      Object.keys(parsed.sections).forEach((s) => {
        this.sections[s] = [];
      });
      if (typeof parsed.sections[0] === Object) {
        parsed.sections.forEach((s, ind) => {
          s[ind].forEach((w) => {
            this.sections[ind].push(w);
          });
        });
      }
    }
  ...
  addWorkout(workout) {
    ...
    if (typeof workout.section !== undefined) {
      this.sections[workout.section].push(workout);
    }
  }
      ...
      for (let w in section) {
        ...
        if (!workout.parent) {
          ...
          if (typeof parent == "string") {
            parent = this.workouts.find(w => w.slug === workout.parent);
          }
          if (parent === undefined) {
  ...
  render() {
    ...
    Object.keys(this.sections).forEach((section) => {
      sectionList[section] = this.sections[section].map((w) => w.slug);
    });
    let output = {
      name: this.name,
  ...
  renderCourse(filter) {
    ...
    const markdown = this.getInsights(filter).reduce((md, insight) => {
      ...
      return md + `${insight.workoutName} | [${pathModule.basename(insight.contentPath)}](${link}) | ${insight.stub ? 'stub' : 'live'}\n`;
    }, '');
