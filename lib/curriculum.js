 ...
 */
  ...
  findElementBySlug(element, slug) {
    switch (element) {
      case "topic":
        return curriculum.topics[slug];
      case "course":
        let [topic, course] = slug.split(":")
        return curriculum.topics[topic].courses[course]
      case "workout":
        let [topic, course, workoutSlug] = slug.split(":")
        return curriculum.topics[topic].courses[course].workouts.find(workout => workout.slug == workoutSlug)
      case "exercise":
        let [topic, course, workoutSlug, exerciseNumber] = slug.split(":")
        let workout = curriculum.topics[topic].courses[course].workouts.find(workout => workout.slug == workoutSlug)
      return workout.exercises[exerciseNumber]
      case "revision":
        let [topic, course, workoutSlug, insightSlug] = slug.split(":")
        let workout = curriculum.topics[topic].courses[course].workouts.find(workout => workout.slug == workoutSlug)
        let insight = workout.insights.find(insight => insight.slug == insightSlug)
        return insight.revisionQuestion
      case "practice":
      let [topic, course, workoutSlug, insightSlug] = slug.split(":")
      let workout = curriculum.topics[topic].courses[course].workouts.find(workout => workout.slug == workoutSlug)
      let insight = workout.insights.find(insight => insight.slug == insightSlug)
      return insight.practiceQuestion
      case "quiz":
        let [topic, course, workoutSlug, insightSlug] = slug.split(":")
        let workout = curriculum.topics[topic].courses[course].workouts.find(workout => workout.slug == workoutSlug)
        let insight = workout.insights.find(insight => insight.slug == insightSlug)
        return insight.quizQuestion
      default:
        return this;
  }
