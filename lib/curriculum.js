...
module.exports = class Curriculum {
  constructor (git) {
    ...
    this.topics = {}
    if (!this.contentPath || !this.standardsPath || !this.wikiPath) {
      throw new Error('Need a file path to content, standards and wiki')
    }
  ...
  read (content, standards) {
    console.log(chalk.black.bgYellowBright(' Parsing operation started. '))

    let contentDirectories = fs.readdirSync(this.contentPath).filter(entry => {
      return (
        fs.statSync(`${this.contentPath}/${entry}`).isDirectory() &&
        entry[0] !== '.'
      )
    })
    let standardsDirectories = fs
      .readdirSync(this.standardsPath)
      .filter(entry => {
        return (
          fs.statSync(`${this.standardsPath}/${entry}`).isDirectory() &&
          entry[0] !== '.'
        )
      })
    ...
    contentDirectories.forEach(topicFolder => {
      ...
      if (topicFolder === '.archived') {
        this.archived = getAllFilesRecursively(topicPath).reduce(
          (archivedTopic, insightPath) => {
            const archivedTopicName = insightPath
              .split('.archived/')[1]
              .split('/')[0]

            if (!archivedTopic[archivedTopicName]) {
              archivedTopic[archivedTopicName] = []
            }

            const archivedInsight = new Insight({
              body: vfile.readSync(insightPath, 'utf8'),
              path: insightPath
            })
      ...
      if (fs.existsSync(readMePath)) {
        ...
        fs
          ...
          .filter(entry => {
            return (
              fs.statSync(`${topicPath}/${entry}`).isDirectory() &&
              entry !== '.git'
            )
          })
          .forEach(courseFolder => {
            ...
            if (fs.existsSync(readMePath)) {
              ...
              fs
                ...
                .filter(entry => {
                  return (
                    fs.statSync(`${coursePath}/${entry}`).isDirectory() &&
                    entry !== '.git'
                  )
                })
                  ...
                  if (fs.existsSync(readMePath)) {
                    ...
                    if (body.length > 0) {
                      ...
                      try {
                        ...
                        workout.setTopic(topic.slug)
                        workout.setCourse(course.slug)
                      } catch (e) {
                        console.log(`Problem with: ${readMePath}`)
                      }
                      ...
                      fs
                        ...
                        .forEach(insightFile => {
                          ...
                          try {
                            ...
                            insight.setTopic(topic.slug)
                            insight.setCourse(course.slug)
                            insight.setWorkout(workout.slug)
                          } catch (e) {
    ...
    console.log(chalk.magenta.bold('Succesfully parsed "curriculum" repository.'))
    // standards
    ...
    standardsDirectories.forEach(topicFolder => {
      ...
      fs
        ...
        .forEach(courseFolder => {
          ...
          fs
            .readdirSync(`${topicPath}/${courseFolder}`)
            .filter(entry => {
            ...
            .forEach(standardsFile => {
              ...
              let standard = new Standard({
                body,
                path: `${topicPath}/${courseFolder}/${standardsFile}`
              })
    ...
    console.log(chalk.magenta.bold('Succesfully parsed "standards" repository.'))

    console.log(chalk.black.bgYellowBright(' Parsing operation finished. '))
  }
  ...
  findElementBySlug (element, slug) {
    ...
    switch (element) {
      ...
      case 'workout':
        return this.topics[topic].courses[course].workouts.find(
          workout => workout.slug === workoutSlug
        )
      case 'exercise':
        let workout = this.topics[topic].courses[course].workouts.find(
          workout => workout.slug === workoutSlug
        )
        let exerciseNumber = insightSlug
        return workout.exercises[exerciseNumber]
      case 'revision':
        let workoutRevision = this.topics[topic].courses[course].workouts.find(
          workout => workout.slug === workoutSlug
        )
        let insightRevision = workoutRevision.insights.find(
          insight => insight.slug === insightSlug
        )
        return insightRevision.revisionQuestion
      case 'practice':
        let workoutPractice = this.topics[topic].courses[course].workouts.find(
          workout => workout.slug === workoutSlug
        )
        let insightPractice = workoutPractice.insights.find(
          insight => insight.slug === insightSlug
        )
        return insightPractice.practiceQuestion
      case 'quiz':
        let workoutQuiz = this.topics[topic].courses[course].workouts.find(
          workout => workout.slug === workoutSlug
        )
        let insightQuiz = workoutQuiz.insights.find(
          insight => insight.slug === insightSlug
        )
  ...
  distributeArchivedInsightsToTopics () {
    Object.keys(this.topics).forEach(topicName =>
      this.topics[topicName].addArchivedInsights(this.archived[topicName])
    )
  }
  ...
  generateCurriculumTreeMarkdown () {
    ...
    for (let topicName in curriculum.topics) {
      ...
      for (let courseName in topic.courses) {
        ...
        data += `* [${toTitleCase(courseName)}](${remote +
          topicName +
          '/' +
          courseName})\n`
...
`
  ...
  readCourseFile ({ topic, path }) {
    ...
    if (this.topics[topicSlug].courses[course.slug]) {
      lastSection =
        Object.keys(this.topics[topicSlug].courses[course.slug].sections)
          .length - 1
    }
    ...
    content
      ...
      .forEach(rawWorkout => {
        ...
        const workoutName = rawWorkout
          .substring(0, rawWorkout.indexOf('\n'))
          .trim()
        const metadata = rawWorkout.substring(
          rawWorkout.indexOf('\n') + 1,
          rawWorkout.indexOf('#') - 1
        )
        ...
        rawWorkout
          ...
          .slice(1)
          .forEach(rawInsight => {
            const insightTitle = rawInsight
              .substring(0, rawInsight.indexOf('\n'))
              .trim()
    ...
    if (this.topics[topicSlug].courses[course.slug]) {
      this.topics[topicSlug].courses[course.slug].workouts.concat(
        course.workouts
      )
      this.topics[topicSlug].courses[course.slug].sections[lastSection].concat(
        course.workouts
      )
    } else {
  ...
  getAllInsights () {
    return [].concat(...this.getAllWorkouts().map(workout => workout.insights))
  }

  getAllExercises () {
    return [].concat(...this.getAllWorkouts().map(workout => workout.exercises))
  }

  getAllWorkouts () {
    return [].concat(...this.getAllCourses().map(course => course.workouts))
  }

  getAllCourses () {
    return [].concat(
      ...Object.values(this.topics).map(topic => Object.values(topic.courses))
    )
  }
}
