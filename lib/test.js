...
*/
const sampleInsight = new Insight({ body: `# Using xargs to build commands.
author: aakarsh
...
Use \`xargs\` to build an d execute commands 
from standard input. This often provides a simpler construct than using explicit loops.

Consider:

Running the command above may fail because there are too many files in \`/pat\`, exceeding the command line maximum length.

Instead, running the following won't[1]:s

Output from the \`find\` will stubstitute for every file found both \`{}\` occurrences.
...
[1:error handling]
To make sure empty files are treated correctly, pass the \`-0\` flag to \`xargs\` to split the lists at **null**
