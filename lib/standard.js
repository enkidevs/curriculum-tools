...
module.exports = class Standards extends Content {
  constructor({ body, path }) {
    // allow vFiles to be passed in without changing interface
  ...
  parse(rawText) {
    ...
    //get slug
    this.slug = lines.find((line) => {return line.startsWith("slug: ")}).replace("slug: ", "");

    const objectivesStartIndex = lines.findIndex((line) => { return line.match(OBJECTIVES_BEGIN_REGEX)}) + 2;
    const objectivesEnd = lines.findIndex((line,i) => {return (i > objectivesStartIndex) ? line.match(SECTION_END_REGEX) : false;})
    const objectives = lines.slice(objectivesStartIndex, objectivesEnd).filter((line) => {return line.trim().length > 1});
    this.objectives = objectives.map((line,i) => {return line.replace("- ", "").replace(/\d+\.\s/, "")});
...
${this.objectives.map((o,i) => {return `${i}. ${o}`}).join("\n")}
