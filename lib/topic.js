...
module.exports = class Topic extends Content {
  ...
  parse(rawText) {
    ...
    yaml.safeLoadAll(rawText, parsedTopicObject => {
      for (const prop in parsedTopicObject) {
        this[prop] = parsedTopicObject[prop];
      }
    });
  ...
  getInsights(filter, { includeArchived = false } = {}) {
    ...
    return includeArchived ? insights.concat(
      filter ? this.archived.filter(insight => filter(insight)) : this.archived
    ) : insights;
  }
