...
module.exports = class GitHub {
  constructor(basePath, branch) {
    if(!basePath) {
      console.log(`No git directory specified, defaulting to ${os.homedir()}/src`);
 ...
 */
  cloneRepos() {
    ...
    if(!fs.existsSync(curriculumPath)) {
      // curriculum
      console.log(`git clone ${CURRICULUM_BASE_URL}/tree/${this.branch}`, {cwd: this.basePath});
      execSync(`git clone ${CURRICULUM_BASE_URL}/tree/${this.branch}`, {cwd: this.basePath});
    }
