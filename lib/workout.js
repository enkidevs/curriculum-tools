 ...
 */
  parseWorkout(text) {
    ...
    data.exercises = exerciseArr.map((exercise)=> { return this.parseExercise(exercise)})
  ...
  render() {
    ...
    if (this.exercises != null && this.exercises.length > 0) {
      for (let exercise of this.exercises) {
        markdown += "---\n## Exercise\n";
        if (typeof exercise['link-type'] != null 
         && typeof exercise['link']      != null) markdown += `${exercise['link-type']}-link: ${exercise['link']}\n`;
        if (typeof exercise['answer']    != null) markdown += `answer: ${exercise.answer.toString().replace(/\t/g, "\n\t")}\n`;
        if (typeof exercise.question     != null) markdown += `### Question\n${exercise.question}\n\n`
      }
  ...
  getStats() {
    ...
    for (let insight of this.insights) {
      ...
      if (!insight.type == "normal" || !insight.practiceQuestion || !insight.revisionQuestion) {
        continue;
      }
