...
module.exports = class Insight extends Content {
  ...
  updateFromAst (ast) {
    ...
    if (insightType === 'exercise') {
      ...
      const gameContent = find(ast, { type: 'section', name: 'GameContent' })
...
function extractQuestionFromAstNode (questionNode) {
  ...
  const answersNodeIndex = childrenNodes.findIndex(node => node.type === 'list')
  if (answersNodeIndex === -1) {
    throw new Error('Invalid question node')
  }
  // removing the list from the AST
  const answers = markdownCompiler
    .compileSync(childrenNodes.splice(answersNodeIndex, 1)[0])
    .split('\n')
    .filter(Boolean)
    .map(i => i.substring(2))
  return {
    rawText,
    text: compileAstNodeToInsight({ children: childrenNodes }),
    answers
  }
}
...
function extractExercise ({ link, linkType }, node) {
