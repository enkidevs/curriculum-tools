...
const { removeFalsy, toTitleCase } = require('./helpers');
...
module.exports = class Topic extends Content {
  ...
  getInsights(filter, { includeArchived = false } = {}) {
    ...
    return includeArchived ? insights.concat(filter ? this.archived.filter(insight => filter(insight)) : this.archived) : insights;
  }
  ...
  renderArchived(filter) {
    ...
    const markdown = wantedArchived.reduce((md, insight) => {
      ...
      return `${md}${insight.workoutName || 'N/A'} | [${pathModule.basename(insight.contentPath)}](${link}) | ${insight.stub ? 'stub' : 'live'}\n`;
    }, '');
